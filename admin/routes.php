...
<div class="container py-5">
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-4">
            <form method="GET" action="<?php echo BASE_URL; ?>admin/routes.php" class="row g-3 align-items-end">
                ...
                <div class="col-lg-4">
                    <button type="submit" class="btn btn-primary w-100">Load Company Routes</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <?php if ($companyId <= 0 || !$company): ?>
        <div class="alert alert-info rounded-4">Choose a company first to manage routes.</div>
    <?php else: ?>
        <div class="row g-4">
            <div class="col-lg-5">
                <div class="card border-0 shadow-sm rounded-4">
                    <div class="card-body p-4">
                        <h5 class="fw-bold mb-3"><?php echo $editRoute ? 'Edit Route' : 'Create New Route'; ?></h5>
                        ...
                        <form action="<?php echo BASE_URL . ($editRoute ? 'actions/update_route.php' : 'actions/create_route.php'); ?>" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" min="1" name="distance_km" class="form-control" value="<?php echo e($editRoute['distance_km'] ?? ''); ?>" required>
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Duration (Minutes)</label>
                                <input type="number" min="1" name="duration_minutes" class="form-control" value="<?php echo e($editRoute['duration_minutes'] ?? ''); ?>" required>
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Base Price</label>
                                <input type="number" step="0.01" min="0" name="base_price" class="form-control" value="<?php echo e($editRoute['base_price'] ?? ''); ?>" required>
                            </div>

                            <div class="mb-4">
                                <label class="form-label">Status</label>
                                <select name="status" class="form-select" required>
                                    <option value="active" <?php echo (($editRoute['status'] ?? 'active') === 'active') ? 'selected' : ''; ?>>Active</option>
                                    <option value="inactive" <?php echo (($editRoute['status'] ?? '') === 'inactive') ? 'selected' : ''; ?>>Inactive</option>
                                </select>
                            </div>

                            <button type="submit" class="btn btn-primary w-100"><?php echo $editRoute ? 'Update Route' : 'Create Route'; ?></button>
                        </form>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-7">
                <div class="card border-0 shadow-sm rounded-4">
                    <div class="card-body p-4">
                        ...
                        <?php if (empty($routes)): ?>
                            <div class="alert alert-info mb-0">No routes found for this company yet.</div>
                        <?php else: ?>
                                ...
                                <table class="table align-middle">
                                    ...
                                    <tbody>
                                        <?php foreach ($routes as $route): ?>
                                            <tr>
                                                <td><?php echo e($route['from_city_name']); ?> → <?php echo e($route['to_city_name']); ?></td>
                                                <td><?php echo e($route['distance_km']); ?> km</td>
                                                <td><?php echo e($route['duration_minutes']); ?> min</td>
                                                <td><?php echo number_format((float)$route['base_price'], 2); ?></td>
                                                <td><span class="badge bg-<?php echo route_status_badge_class($route['status']); ?>"><?php echo e(ucfirst($route['status'])); ?></span></td>
                                                <td class="text-end">
                                                    <a href="<?php echo BASE_URL; ?>admin/routes.php?company_id=<?php echo e($companyId); ?>&edit=<?php echo e($route['id']); ?>" class="btn btn-sm btn-outline-primary">Edit</a>
                                                    <form action="<?php echo BASE_URL; ?>actions/delete_route.php" method="POST" class="d-inline">
                                                        ...
                                                        <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete this route?')">Delete</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
